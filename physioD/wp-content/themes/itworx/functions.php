...
wp_enqueue_script('jquery', TEMPLATE_URL . '/lib/scripts/jquery.js');
...
function phi_cycle_slider($termId,$size){
	...
	$result.= '</div>';
	// Slide nav
	$result.= '<div class="slide-nav-markers">';
	$result.= '</div>';
	$result.= '</div>';
...
if ($columns == 2){$imagewidth = 'half';  $imageheight = get_option('phi_half_image_height'); $divclass = 'one-half'; $imagewrap = 'image-wrap';}
if ($columns == 3){$imagewidth = 'third'; $imageheight = get_option('phi_third_image_height'); $divclass = 'one-third'; $imagewrap = 'image-wrap';}
if ($columns == 4){$imagewidth = 'fourth';$imageheight = get_option('phi_fourth_image_height'); $divclass = 'one-fourth'; $imagewrap = 'image-wrap';}
if ($columns == 5){$imagewidth = 'fifth'; $imageheight = get_option('phi_fifth_image_height'); $divclass = 'one-fifth'; $imagewrap = 'image-wrap';}
...
$result.= get_option('phi_trans_readmore');
$result.= '</a> </div></div>';
	}
	wp_reset_query();
	return $result;
...
function phi_home_article(){
			...
			$excerpt = get_post_meta($post->ID,'phi_customexcerpt',true);
				echo '<div class="one-half" id="phi_home_article1">';
	...
	function phi_home_article2(){
			...
			if (have_posts()) : the_post();
			$excerpt = get_post_meta($post->ID,'phi_customexcerpt',true);
				echo '<div class="one-half last" id="phi_home_article2">';
					echo '<h2>'.get_the_title().'</h2>';
...
echo '<div class="one-fourth '.$position.'">';
echo phi_post_image('featured','return','permalink');
echo '<h3><a href="'.get_permalink().'">'.get_the_title().'</a></h3>';
echo '<p>'.phi_custom_excerpt($excerpt,$charlimit).'</p>';
echo '<a href="'.get_permalink().'" class="button">'.get_option('phi_trans_readmore').'</a>';
echo '</div>';
...
function phi_news_list($pager,$format){
	...
	$args = array( 'post_type' => 'news',
         ...
         'order' => 'DESC',
			...
			$result.= '<h4 class="post_date">'.get_the_time('d/m/Y').'</h4>';
			$result.= '<h2><a href="'.get_permalink().'">'.get_the_title().'</a></h2>';
			$result.= '<p>'.phi_custom_excerpt($text,$charlimit).'</p><p><a href="'.get_permalink().'" class="button">'.get_option('phi_trans_readmore').'</a></p>';
			$result.= '</div></div>';
...
function phi_testimonial_list($pager,$format){
	...
	$args = array( 'post_type' => 'testimonial',
         ...
         'order' => 'DESC',
			...
			$result.= '<strong><a href="'.get_permalink().'" class="button">'.get_option('phi_trans_readmore').'</a></strong>';
			$result.= '</div>';
			endwhile;
...
function phi_event_list($format){
	...
	$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
                               ...
                               'order' => 'ASC',
									...
									$result.= '<h4 class="post_date">'.get_the_date(). ' '. get_option('phi_trans_at').' ' .get_the_time().'</h4>';
									$result.= '<h2><a href="'.get_permalink().'">'.get_the_title().'</a></h2>';
									$result.= '<p>'.get_the_excerpt().'</p>';
									$result.= '<strong><a href="'.get_permalink().'" class="button">'.get_option('phi_trans_readmore').'</a></strong>';
									$result.= '</div></div>';
							...
							$result.= '<div class="tabcontent" id="tab0">';
							 ...
							 while (have_posts()) : the_post(); 
								...
								if (!empty($post->post_content)):
									$result.= '<h6><span>'.get_option('phi_trans_answer').'</span></h6>';
									$result.= get_the_content_formatted();
									$result.= '<p>'.get_option('phi_trans_regards').'</p>';
							...
							if (have_posts()) : 
								...
								$result.= '<p>'.get_the_excerpt().'</p>';
								 if (!empty($post->post_content)):
									...
									$result.= '<h6>'.get_option('phi_trans_answer').'</h6>';
...
if(isset($_POST['phi_name'])) {
		...
		if(!isset($hasError)) {
			...
			$formString.='</li>';
			$formString.='<li>';
			$formString.= '<input type="submit" id="submitbutton" value="'.get_option('phi_trans_submitreply').'"  />';
			$formString.='</li>';
			$formString.='</ul>';
			$formString.='</form>';
			$formString.='</div>';
...
if($video){$lightbox = $video;}
			...
			$result.= '<a href="'.get_permalink().'" class="button">'.get_option('phi_trans_readmore').'</a>';
			$result.= '</div>';
			$result.= '</div>';
			}
...
if (have_posts()): while (have_posts()) : the_post(); 
			...
			$result.= '<h4 class="post_date">';
						...
						$result.= get_option('phi_trans_postedon');
						$result.= get_the_date();
						}
			$result.= '</h4>';
			$result.= '<h2><a href="'.get_permalink().'">';
			...
			$result.= '<p class="post_meta">';
						if($hideauthor == false){
						$result.= get_option('phi_trans_postedby').' ';
						$result.= get_the_author().' ';
						}
						if($hidecategories == false){
						$result.= get_option('phi_trans_incategory').' ';
						
						foreach((get_the_category()) as $category) { 
    					$result.=  '<a href="'.get_permalink().'">'.$category->cat_name.'</a>'; 
} 
						}
			$result.= '</p>';
			...
			$result.= '<p><a href="'.get_permalink().'" class="button">'.get_option('phi_trans_readmore').'</a></p>';
						if ( comments_open() ){
						//$result.= comments_popup_link ( '0 '.get_option('phi_trans_responses'), '1 '.get_option('phi_trans_response'), '% '.get_option('phi_trans_responses'), 'comments_link', '');
						}
						$result.= '</div></div>';
