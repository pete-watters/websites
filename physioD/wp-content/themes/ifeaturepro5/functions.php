			...
			<footer>
				...
				<div class="comment-meta commentmetadata">
					...
					<?php
						...
						printf( '%1$s ' . __( 'at', 'cyberchimps' ) . ' %2$s', get_comment_date(), get_comment_time() ); ?>
					</time></a>
...
function ifeature_post_tags( $tags ) {
	$tag = trim( $tags, __( 'Tags:', 'cyberchimps' ) );
	$tag = explode( ',', $tag );
	$tag = implode( ' ', $tag );
	return $tag;
}
...
//add imenu section
function ifeature_add_sections( $original ) {
	$new_section[][2] = array( 	'id' => 'cyberchimps_imenu_section',
...
add_filter( 'cyberchimps_sections_filter', 'ifeature_add_sections', 2 );

//add fields 2nd key is the position in that section
function ifeature_add_fields( $original ) {
	$new_field[][]	= array( 	'name' => __('Top Header Bar', 'cyberchimps'),
														'id' => 'top_header_bar',
														'type' => 'toggle',
														'std' => 1,
	...
	$new_field[][] = array( 	'name' => __('Menu Home Icon', 'cyberchimps'),
														'id' => 'menu_home_button',
														'type' => 'toggle',
														'std' => 1,
	...
	$new_field[][1] = array(	'name' => __( 'Add Telephone Number', 'cyberchimps' ),
														'id' => 'contact_tel_number',
														'type' => 'toggle',
														'std' => 1,
														'section' => 'cyberchimps_header_details_section',
														'heading' => 'cyberchimps_header_heading'
													);
	$new_field[][2] = array(	'name' => __( 'Telephone Number', 'cyberchimps' ),
														'id' => 'tel_number',
														'class' => 'contact_tel_number_toggle',
														'type' => 'text',
														'std' => __( 'Enter your phone number', 'cyberchimps' ),
														'section' => 'cyberchimps_header_details_section',
														'heading' => 'cyberchimps_header_heading'
												);
	$new_field[][3] = array(  'name' => __('Add Email Address', 'cyberchimps'),
														'id' => 'contact_email',
														'type' => 'toggle',
														'std' => 1,
														'section' => 'cyberchimps_header_details_section',
	...
	$new_field[][4] = array(	'name' => __('Email', 'cyberchimps'),
														'id' => 'email',
														'class' => 'contact_email_toggle',
														'type' => 'text',
														'std' => 'name@domain.com',
														'section' => 'cyberchimps_header_details_section',
														'heading' => 'cyberchimps_header_heading'
...
add_filter( 'cyberchimps_field_filter', 'ifeature_fields_filter', 3 );

// Add new conatct information to header
function ifeature_contact_details_header() {
	$contact = '<div class="cc-telephone-number">' . cyberchimps_get_option( 'tel_number', __( 'Enter your phone number', 'cyberchimps' ) ) . '<img src="' .get_template_directory_uri() . '/images/phone.png" alt="phone" /></div>';
	$contact .= '<div class="cc-email">' . cyberchimps_get_option( 'email', 'name@domain.com' ) . '<img src="' . get_template_directory_uri() . '/images/email.png" alt="email" /></div>';
	return $contact;
}
add_filter( 'cyberchimps_header_contact', 'ifeature_contact_details_header', 2 );
...
add_filter( 'cyberchimps_fallback_menu_filter', 'ifeature_add_home_menu', 10, 2 );
?>