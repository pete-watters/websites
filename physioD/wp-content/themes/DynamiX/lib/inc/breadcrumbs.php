...
// Slightly modified script of dimox_breadcrumbs 
  ...
  $name = __('Home', 'DynamiX' ); //text for the 'Home' link
  $before = '<li>';  
  $current_before = '<li class="current_page_item">';
  $after = '</li>';
  ...
  if ( !is_home() && !is_front_page() || is_paged() ) {
    ...
    if ( is_category() ) {
      ...
      echo $current_before . get_the_time('d') . $after;
      ...
      echo $current_before . get_the_time('Y') . $after;
	...
	if( !is_object( $cat_parent ) ) {
      ...
      echo $current_before . __('Search results for ', 'DynamiX' ).'&#39;' . get_search_query() . '&#39;' . $after;
 
    } elseif ( is_tag() ) {
      echo $current_before . __('Posts Tagged', 'DynamiX' ).'&#39;';
      single_tag_title();
      echo '&#39;' . $after;
      ...
      echo $current_before . __('Articles posted by ', 'DynamiX' ) . $userdata->display_name . $after;
 
    } elseif ( is_404() ) {
      echo $current_before . __('Error ', 'DynamiX' ) . '404' . $after;
    }
 
    if ( get_query_var('paged') ) {
      if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() );
      echo $current_before . __('Page') . ' ' . get_query_var('paged') . $after;
      if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() );
    }
