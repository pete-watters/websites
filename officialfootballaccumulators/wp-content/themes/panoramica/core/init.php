...
function cpotheme_setup(){
	//Set core variables
	cpotheme_update_option('cpo_core_version', '1.3.0');
...
function cpotheme_add_scripts( ){
	...
	if(!is_admin()){
		...
		wp_enqueue_script('script_general_lightbox', $scripts_path.'jquery_prettyphoto.js');
		wp_enqueue_script('script_general_admin', $scripts_path.'general.js');
...
function cpotheme_add_styles(){
    ...
    //Common styles
	wp_enqueue_style('style_colorpicker', $stylesheets_path.'prettyphoto.css');
...
require_once($core_path.'shortcodes/shortcode_slider.php');
