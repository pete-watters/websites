...
if(!function_exists('cpotheme_shortcode_fblike')){
	function cpotheme_shortcode_fblike($atts, $content = null){
		...
		$output = '<div class="social_like" style="float:'.$button_position.'; width:'.$width.'px; height:'.$height.'px;">';
	...
	add_shortcode('fb_like', 'cpotheme_shortcode_fblike');
}

/* Tweet Button Shortcode */
if(!function_exists('cpotheme_shortcode_tweet')){
	function cpotheme_shortcode_tweet($atts, $content = null){
		$attributes = extract(shortcode_atts(array(
			'url' => '',
			'style' => 'none',
			'width' => 450,
		...
		$output = '<div class="social_tweet" style="float:'.$button_position.'; width:'.$width.'px; height:'.$height.'px;">';
	...
	add_shortcode('tweet', 'cpotheme_shortcode_tweet');
}

/* Plus One Button Shortcode */
if(!function_exists('cpotheme_shortcode_gplus')){
	function cpotheme_shortcode_gplus($atts, $content = null){
		$attributes = extract(shortcode_atts(array(
			'counter' => '',
			'style' => '',
			'width' => 450,
		...
		$output = '<div class="social_gplus" style="float:'.$button_position.'; width:'.$width.'px; height:'.$height.'px;">';
	...
	add_shortcode('gplus', 'cpotheme_shortcode_gplus');
}

//Displays a Tweet button
//add_action('footer', 'cpotheme_shortcode_scripts');
function cpotheme_shortcode_scripts(){
